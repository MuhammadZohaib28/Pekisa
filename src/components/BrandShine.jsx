...
const BrandShine = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row justify-around items-start p-20 md:pb-28   pl-4 pr-4 md:pl-24 md:pr-24 gap-12 md:gap-28 bg-[#B4D4FF]">
      ...
      <div className="flex flex-col items-center gap-12 md:gap-20  flex-1">
        {productsData.map((product, index) => (
          <div
            className="flex justify-between items-start gap-8 bg-white border-2 border-gray-400 rounded-md p-2 sm:p-4"
            key={index}
          >
            <h1 className="text-5xl">{product.icon}</h1>
            <div className="flex flex-col gap-4">
              <Link to={product.url} className="flex items-center gap-5">
                <h1 className="font-poppins text-lg sm:text-2xl text-gray-800 font-semibold">
                  {product.title}{" "}
                </h1>
                <FaArrowRightLong className="text-xl" />
              </Link>
              <p className="font-poppins text-sm">{product.description}</p>
            </div>
          </div>
        ))}
      </div>
